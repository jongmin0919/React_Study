...
1. 리액트는 JSX(JSeXtension)문법을 사용하며 주로 함수형 컴포넌트를 메인으로 개발하는 라이브러리 입니다.
...
4. 어떻게(how)에 초점을 맞춘 명령형 프로그래밍은 코드의 장문화를 불러오는대에 반해
   ...
   캡슐화 하는대에 용이합니다.
5. 가상 DOM을 사용하여 UI 업데이트 성능을 최적화합니다. 가상 DOM은 실제 DOM과 동기화되는 가상의 트리 구조입니다. 
   리액트는 상태(State) 변화에 따라 가상 DOM을 업데이트하고, 실제 DOM과 비교하여 변경된 부분만 업데이트합니다. 
   이를 통해 DOM 조작 횟수를 최소화하고 성능을 향상시킵니다.
...
<!--9월 16일 공부 : 1-3 나의 첫 리액트 프로젝트  (P58-P83) -->
...
※ DOM의 개념 
1. 우선 DOM(Document Object Model)은 HTML, XML등의 구조, 컨텐츠, 스타일 요소 등을 구조화시켜 표현하여 프로그래밍 언어(여기선 javascript)가
해당 웹문서에 접근하여 요소들을 읽거나 조작 할수 있도록 API를 제공하는 일종의 인터페이스 입니다. 즉 프로그래밍 언어와 웹문서 사이에서
어떻게 웹문서의 구조나 컨텐츠를 읽거나 조작을 하여야 하는지를 제공하는 설명서 정도로 보면 될것 같은데요.
2. 이러한 DOM은 '트리 구조' 로 구현이 됩니다. 그 이유는 웹페이지 구축에 있어서 최상단의 문서 노드(root)로부터 
하위의 요소노드, 어트리뷰트노드, 텍스트노드를 순차적으로 도식화 할수 있고, 보다 부모와 자식 요소간의 관계를 쉽게 파악할 수 있기 때문입니다.
3. 자바들은 이 DOM의 노드들을 웹브라우저에서 제공하는 window 객체의 document 프로퍼티를 통해 해당 노드들에 접근 및 조작이 가능합니다. 

★ 웹 브라우저의 렌더링 과정 ★
웹브라우저는 렌더 엔진을 이용해 웹 문서(html)와 스타일 시트(css)를 파싱하는 과정으로 각각의 트리인 DOM Tree와 Style Rules
를 생성한 후 이 둘을 필요한 부분만 결합하여 최종적으로 Render Tree를 생성하고, 이후 각 노드들이 화면에 표시되어야 하는 크기와 위치를 
계산하고(Layout), 색상을 입히는 painting 작업을 통해 최종적으로 사용자에게 렌더링된 화면을 표시(display) 합니다.

  ※ DOM을 이용한 간단한 노드 생성 및 조작법
  - 노드 생성
  ```
   const div = document.createElemt('div')
   // 새로운 div 노드를 메모리상에 생성한 후 해당 노드를 div 변수에 할당합니다. 하지만 웹페이지 상에는 아직 추가되지 않은 상태입니다.
  ```
  - 생성된 노드를 부모 노드에 붙임
  ```
   document.body.append(div);
   // 생성된 자식 노드를 부모 노드(body)에 접근하고 붙여줍니다.
  ```
  - 생성된 노드를 수정할 경우
  ```
   div.textContent = 'Hello World!'
   // 생성된 노드가 할당 되어있는 변수의 내용을 다음 문구로 작성시 실제 생성된 문서 상의 div 안에 해당 텍스트가 생성됩니다.
  ```

※ ReactDOM과 가상돔의 등장 이유와 개념

1. 기존의 DOM으로 직접 웹문서를 조작하는 방식은 특정 정보를 지속적으로 보여주는 정적인 웹사이트나, DOM 조작이 적은 웹 페이지의 경우에는 크게 문제가 되지 않았으나, 요즘과 같이 실시간 정보를 반영하는 웹페이지 특성으로 기존의 DOM 렌더링 방식에 한계를 느끼게 되어(노드가 반영 될때마다 리플로잉과 리페인팅 작업이 순차적으로 발생 되어 웹페이지에 렌더링 정보가 표시 되는 시간을 많이 잡아먹게 된다.) 대규모 프로젝트에 효율적으로 코드 관리를 위해서 컴포넌트(독립성 및 재사용성 등을 강조한 개념)기반의 UI 개발을 지원하는 프레임워크(라이브러리) 중에서 리액트의 가상 돔(Virtual DOM)의 개념을 활용해 신속하게 UI를 업데이트 하고, 필요한 컴포넌트를 언제든지 호출이 가능하고, 코더의 반복적인 코드 작성 없이 사용할 수 있게 해줍니다.

★ 프레임워크 VS 라이브러리 ★
  프레임워크 : 특정 기능을 수행하는 함수, 클래스 등을 모듈화하고 개발을 위한 기본 구조와 규칙을 제공하는 도구이며 개발자는 해당 도구(프레임워크)가 제공하는 규칙과 인터페이스에 따라 코드를 작성해야 합니다. 
  라이브러리 : 재사용이 가능한 코드의 집합이며 개발자가 필요한 기능을 호출하여 사용할 수 있고, 프레임워크와 달리 개발자가 직접 코드의 흐름과 제어를 관리 할수 있습니다.

★ 리액트 돔의 설치 및 확인 방법 ★
 해당 ReactDOM을 사용하기 위해서는 별도 명령어를 패키지 매니저로 설치 할수 있으나, 리액트를 설치하면 같이 자동으로 모듈에 포함되는데, node_modules 폴더 안에 react 파일과 함께 react-dom 파일도 설치 되는데 이 안에 가상 돔(Virtual DOM)이 포함되어 있습니다.
 또한 해당 리액트 돔의 버전 정보는 package.json에서 확인이 가능합니다.

2. 리액트 가상 돔의 원리는 실제 돔 트리를 메모리에 복사를 해두었다가(이것이 React Virtual DOM, 가상 렌더 트리) 사용자가 웹페이지의 특정 콘텐츠를 조작할 시 변경 된 내용을 가상 렌더 트리에 반영한 실제 돔 트리와 비교하여 바뀐 부분(노드, 리액트는 컴포넌트)만 반영시키는 원리입니다.
이럴 경우 기존의 순차적으로 발생 사항을 적용하던 DOM 방식보다 훨씬 더 부드럽고 빠르게 바뀐 정보를 사용자에게 노출시킬 수 있게 됩니다. 


※ 리액트의 핵심, Props와 State

1. 리액트에서 데이터를 다룰때 중요한 기법인 Props와 State의 개념은 다음과 같습니다.
- Props : 부모 컴포넌트에서 자식 컴포넌트로 데이터를 전달하는 방법이며, 읽은 데이터를 사용하거나 렌더링 합니다.


- State : 컴포넌트의 내부 데이터를 관리하고 업데이트하는 방법입니다. 또한 리액트의 각각의 컴포넌트의 현재 상태를 의미하기도 하는데 특정 컴포넌트 안에서 데이터를 유동적으로 다루거나 변경된 내용이 화면에 반영되어야 할때 사용합니다.
  State를 다루기 위해서는 우선 Hook중 useState 함수를 임포트 해와야 합니다.
  ```
  Import { useState } from 'react';
  ```
  다음으로 함수 컴포넌트 내에서 상태를 선언하고 초기값을 설정 할수 있도록 다음과 같은 코드를 작성합니다.
  ```
  const [데이터의 초기값이 들어간 변수, 데이터를 수정할 수 있는 조작 함수] = useState(데이터의 초기값)
  ```
  설정이 완료 되었다면 데이터의 변수는 데이터 값이 바뀌길 원하는 JSX 부분에 기입합니다.
  기입한 후에 해당 함수의 실행에 따라 counter의 값이 바뀌게 됩니다.
  ```
  <Label>{counter}</Label>
  ```
  이제 특정 이벤트를 일으키는 함수가 발생 되었을 때 설정해놓은 조작 함수를 활용해 데이터의 상태를 변화시켜주도록 합니다.
  ```
  ...
  // add 함수나 sub 암수가 실행 될때 안에 있는 조작 함수를 호출시켜 현재의 상태에서 1 증가시키거나 감소시키도록 
  코드를 작성해줍니다.

  <Button onClick={sub}>-</Button>
  <Label>{counter}</Label>
  <Button onClick={sub}>-</Button>
  //이제 특정 Button 컴포넌트를 클릭 할 때마다  이벤트 핸들러 함수가 실행되고, 안에 있는 조작 함수를 통해
  Label 컴포넌트에 있는 counter의 상태가 변경(더하거나 빼지거나)한 후 Label 컴포넌트를 렌더링 하고 Return을 해주면서
  변경된 UI를 사용자에게 보여줍니다.
